...
//        this->update();
    ...
    void WayPointItem::RefreshToolTip()
    {
        ...
        setToolTip(QString("路径点编号： %1\n描述： %2\n经纬度坐标： %4\n海拔高度： %5 m\n偏航角： %6 deg").arg(QString::number(WayPointItem::number)).arg(description).arg(coord_str).arg(QString::number(altitude)).arg(QString::number(heading)));
    }
